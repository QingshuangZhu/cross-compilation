...
NDK=/opt/toolchain/command-line-tools/sdk/default/openharmony/native    # OpenHarmony NDK path
ARCH=aarch64_ohos                            # aarch64, armv7a, etc.
API=29                                 # OHOS API level
PREFIX=$HOME/opt/ohos-arm64                 # Install prefix
...
build_fcgi() {
  ...
  if [ ! -f "${PREFIX}/include/fastcgi.h" ]; then
    ...
    ./configure --host="${TARGET_TRIPLE}" --prefix="${PREFIX}" --with-pic --with-sysroot="${SYSROOT}"
    make -j"${JOBS}"
    make install
  else
...
build_openssl() {
  ...
  if [ ! -f "${PREFIX}/include/openssl/ssl.h" ] || [ ! -f "${PREFIX}/lib/libcrypto.a" ] || [ ! -f "${PREFIX}/lib/libssl.a" ]; then
    make clean || true
    ./Configure "${OPENSSL_TARGET}" --prefix="${PREFIX}" -D__MUSL__ no-shared no-unit-test
    make -j"${JOBS}"
    make install_sw
  else
