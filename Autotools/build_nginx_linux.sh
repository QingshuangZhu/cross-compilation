...
ARCH=x86_64                            # aarch64, armv7a, etc.
PREFIX=$HOME/opt/linux-x86_64               # Install prefix
...
NGINX_VER="1.29.2"                     # nginx version
...
export CFLAGS="-fPIC -O2 -pipe -D_GNU_SOURCE"
export CXXFLAGS="-fPIC -O2 -pipe -D_GNU_SOURCE"
...
build_nginx() {
  echo "=== Building nginx ==="
  if [ ! -d "${BUILD_DIR}/nginx-${NGINX_VER}" ]; then
    extract "${SRC_DIR}/nginx-${NGINX_VER}.tar.gz" "${BUILD_DIR}"
  fi
...
main() {
    ...
    echo "Copy ${PREFIX} to device, set proper permissions and run ${PREFIX}/sbin/nginx -c /path/to/nginx.conf"
}
